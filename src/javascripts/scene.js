...
export default class Scene {
    ...
    return new Promise(resolve => {
      ...
      Promise.all([
        ...
        this.controller.addEventListener( 'connected', () => {
          // this.paddle_mesh = new Object3D();
          for (let i = 0; i < this.paddle.children.length; i++) {
            // this.paddle.children[i].scale.set(0.024, 0.024, 0.024);
            // this.paddle.children[i].position.set(0, -1, 0.5);
            // this.paddle.children[i].rotation.set(-0.78,0,0);
          }
  ...
  onBallPaddleCollision(point) {
    ...
    this.physics.onBallPaddleCollision({body: this.physics.ball, target: this.controller});
  ...
  updateControls() {
    ...
    if (this.hitTween && this.hitTween.isActive()) {
      ...
      this.controller.position.copy(newPos);
    } else if (pos) {
      this.controller.position.copy(pos);
    }
  ...
  animate() {
    ...
    if (this.ball) {
      ...
      dist.subVectors(this.ball.position, this.controller.position);
      // console.log(this.ball.position, this.controller.position)
      if (
